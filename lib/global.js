...
import { THEMES, initDarkMode, loadDarkModeFromCookies } from '@/themes/theme'
...
export function GlobalContextProvider({ children }) {
  ...
  const darkModeFromCookies = loadDarkModeFromCookies()
  let initialDarkMode

  if (darkModeFromCookies === 'true') {
    initialDarkMode = true
  } else if (darkModeFromCookies === undefined) {
    initialDarkMode = BLOG.APPEARANCE === 'dark'
  } else {
    initialDarkMode = false
  }
  const [isDarkMode, updateDarkMode] = useState(initialDarkMode)
  // const [isDarkMode, updateDarkMode] = useState(loadDarkModeFromCookies()) // 默认深色模式
